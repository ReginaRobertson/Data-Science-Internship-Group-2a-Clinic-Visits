# Project workflow — group2a

**Domain:** Clinic visits &nbsp;|&nbsp; **Paired with:** group2b &nbsp;|&nbsp; **Reads:** `raw_clinic` &nbsp;|&nbsp; **Writes:** `group2a`
...
Bad: "Analyse clinic data."

Good: "Which departments have the longest patient wait times, and how does that vary by visit type and over time? We will answer this by comparing department_name across the full 2024–2025 period, broken down by month. Success means a stakeholder can identify the top three and the bottom three at a glance and see how each has moved over time. We are not modelling or forecasting."
...
Work through `week2_profiling_clinic.sql` from top to bottom. Do not skip to the interesting queries.
...
> Share data-quality findings openly with group2b. If you find a broken column, say so — everyone hits the same potholes. Your **analysis** stays yours.
...
- [ ] A breakdown by department_name
...
- [ ] Added the pages you actually need (suggested: Wait time overview, Department comparison, Visit type and trend)
...
| Database | `internship` |
| Username | `group2a` |
| Read from | `raw_clinic` |
| Write to | `group2a` |
| Fact table | `raw_clinic.visits` — 25,100 rows, one patient visit |
| Dimensions | patients (900), doctors (40), departments (8) |
